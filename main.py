...
CLIENT_ID   = os.environ.get("ANGEL_CLIENT_ID", "APNA_CLIENT_ID")
API_KEY     = os.environ.get("ANGEL_API_KEY", "APNA_API_KEY")
TOTP_SECRET = os.environ.get("ANGEL_TOTP_SECRET", "APNA_TOTP")
MPIN        = os.environ.get("ANGEL_MPIN", "APNA_MPIN")

bot = None
if BOT_TOKEN and BOT_TOKEN != "APNA_BOT_TOKEN":
    try:
...
# ─────────────────────────────────────────
app = Flask(__name__)
CORS(app)  # ← Local HTML file se access allow karta hai
...
def generate_signal(price, change, pchg, pcr, vix):
    ...
    reasons = []
    if pcr < 0.7:
        score += 1; reasons.append("✅ PCR Bullish")
    elif pcr > 1.3:
        score += 1; reasons.append("✅ PCR Bearish")
    else:
        reasons.append("❌ PCR Neutral")
    if change > 0:
        score += 1; reasons.append("✅ Price +ve")
    elif change < 0:
        score += 1; reasons.append("✅ Price -ve")
    else:
        reasons.append("❌ Price Flat")
    if abs(pchg) > 0.4:
        score += 1; reasons.append("✅ Momentum Strong")
    else:
        reasons.append("❌ Momentum Weak")
    if vix:
        if vix < 15:
            score += 1; reasons.append("✅ VIX Low")
        elif vix > 22:
...
/alert ON/OFF""")
    ...
    def nifty_cmd(m):
        ...
        if not price:
            bot.reply_to(m, "🔴 Data unavailable"); return
...
📉 Trend : {trend}""")
    ...
    def pcr_cmd(m):
        ...
        vix = get_vix()
        if not pcr:
            bot.reply_to(m, "🔴 PCR unavailable"); return
...
VIX : {vix or 'N/A'}{oi_text}""")
    ...
    def signal_cmd(m):
        ...
        vix = get_vix()
        if not price:
            bot.reply_to(m, "🔴 Data failed"); return
        sig = generate_signal(price, change, pchg, pcr, vix)
        if isinstance(sig, str):
            bot.reply_to(m, sig); return
...
⚠️ Educational only!""")
    ...
    def mcx_cmd(m):
        d = get_mcx_data()
        if not d:
            bot.reply_to(m, "🔴 MCX unavailable"); return
...
💱 USD/INR: {d['usd_inr']}""")
    ...
    def vix_cmd(m):
        vix = get_vix()
        if not vix:
            bot.reply_to(m, "🔴 VIX unavailable"); return
...
Tip   : {tip}""")
    ...
    def status_cmd(m):
        ...
        data = get_all_market_data()
        n = data['nifty']; bn = data['banknifty']; mcx = data['mcx']
        msg = f"""📊 GOAT PRO STATUS
⏰ {data['timestamp']}
{'🟢' if n.get('change',0)>0 else '🔴'} NIFTY   : {n.get('price','--')}
{'🟢' if bn.get('change',0)>0 else '🔴'} BANKNIFTY: {bn.get('price','--')}
😨 VIX     : {data['vix'] or '--'}
📊 PCR     : {data['pcr'] or '--'}
"""
        if mcx:
...
if __name__ == "__main__":
    ...
    if all([CLIENT_ID != "APNA_CLIENT_ID", API_KEY != "APNA_API_KEY",
            TOTP_SECRET != "APNA_TOTP", MPIN != "APNA_MPIN"]):
