...
# Map our internal symbols to Yahoo Finance tickers (index/futures approximations)
_YF_SYMBOL_MAP = {
    "NIFTY": "^NSEI",
    "BANKNIFTY": "^NSEBANK",
...
_YF_CACHE_TTL = 30  # seconds — avoid hammering yfinance


def _get_yfinance_ltp(internal_symbol: str) -> Optional[float]:
    """Fallback LTP fetch from Yahoo Finance when Angel One is unavailable.
    Cached briefly to avoid excessive calls. Data may be delayed."""
    if not _YFINANCE_AVAILABLE:
        return None

    yf_ticker = _YF_SYMBOL_MAP.get(internal_symbol.upper())
    if not yf_ticker:
        return None

    now = time.time()
    with _yf_cache_lock:
        cached = _yf_cache.get(yf_ticker)
        if cached and (now - cached["time"]) < _YF_CACHE_TTL:
            return cached["value"]
    ...
    try:
        ...
        with _yf_cache_lock:
            _yf_cache[yf_ticker] = {"value": price, "time": now}
        return price
    except Exception as e:
        logger.error(f"yfinance fallback error for {internal_symbol}: {e}")
        return None


def _get_ltp_with_fallback(exchange: str, symbol: str, internal_name: str) -> tuple[Optional[float], str]:
    """Try Angel One first; fall back to yfinance if it returns None.
    Returns (value, source) where source is 'ANGEL_ONE' or 'YFINANCE_TEMPORARY_FALLBACK' or 'UNAVAILABLE'."""
    value = angel_client.get_ltp(exchange, symbol)
    if value is not None:
        return value, "ANGEL_ONE"

    fallback_value = _get_yfinance_ltp(internal_name)
    if fallback_value is not None:
        return fallback_value, "YFINANCE_TEMPORARY_FALLBACK"
...
def price_poller() -> None:
    ...
    Falls back to yfinance (delayed data) if Angel One is unavailable
    (e.g. account temporarily blocked). Fallback source is clearly labeled."""
    while True:
        try:
            if config.get_market_status() in ("OPEN", "PRE_OPEN"):
                nifty, nifty_src = _get_ltp_with_fallback("NSE", config.NIFTY_SYMBOL, "NIFTY")
                banknifty, banknifty_src = _get_ltp_with_fallback("NSE", config.BANKNIFTY_SYMBOL, "BANKNIFTY")
                finnifty = angel_client.get_ltp("NSE", config.FINNIFTY_SYMBOL)
                sensex, sensex_src = _get_ltp_with_fallback("BSE", config.SENSEX_SYMBOL, "SENSEX")
                ...
                vix, vix_src = _get_ltp_with_fallback("NSE", config.VIX_SYMBOL, "VIX")
                if vix is None:
                ...
                # Track which source served NIFTY/BankNifty/Sensex/VIX so the
                # dashboard/API can clearly label temporary fallback data.
                updates["data_source"] = {
                    "nifty": nifty_src,
                    "banknifty": banknifty_src,
                    "sensex": sensex_src,
                    "vix": vix_src,
                }
